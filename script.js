...
function orderMessage(){const summary=pricing();const lines=cart.map(item=>{const p=productById(item.productId);return `• ${p.name} | מידה ${item.size} | ${item.backPrint?'הדפסה בגב':'ללא הדפסה בגב'} | כמות: ${item.quantity}`;}).join('%0A');const delivery=$('#delivery-method')?.value==='delivery'?'משלוח (+5 ש״ח)':'איסוף עצמי';return `היי אשכנזי! אני רוצה להזמין:%0A${lines}%0A%0Aקבלת הזמנה: ${delivery}%0Aסה״כ: ${money(summary.total)}%0A%0Aשם מלא:%0Aטלפון:%0Aכתובת / זמן איסוף:`;}
function checkout(){if(!cart.length){alert('הסל ריק — הוסיפו חולצה כדי לשלוח הזמנה.');return;}pendingOrder={items:cart,createdAt:new Date().toISOString()};localStorage.setItem('ashkenazi-last-order',JSON.stringify(pendingOrder));const url=`https://wa.me/${WHATSAPP_NUMBER}?text=${orderMessage()}`;if(WHATSAPP_NUMBER.includes('X')){alert('האתר מוכן לוואטסאפ. החלף את WHATSAPP_NUMBER בראש script.js למספר העסק לפני פרסום.');}else window.open(url,'_blank','noopener');cart=[];coupon={active:false,value:0};saveCart();closeCart();renderCart();$('#success-modal').classList.add('is-open');$('#success-modal').setAttribute('aria-hidden','false');}
...
$('#review-form').addEventListener('submit',event=>{event.preventDefault();const data=new FormData(event.currentTarget);reviews.unshift({name:data.get('name'),text:data.get('text'),rating:Number(data.get('rating'))});localStorage.setItem('ashkenazi-reviews',JSON.stringify(reviews));event.currentTarget.reset();closeModal('#review-modal');renderReviews();});
